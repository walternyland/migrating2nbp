 ...
 * 
* <P>
 * Related preferences are grouped together and placed in a single pane of a
 * <tt>JTabbedPane</tt>, which corresponds to an implementation of
 * {@link PreferencesEditor}. Values are pre-populated with current values for
 * preferences.
 * 
* <P>
 ...
 * 
* <P>
 * Preferences are not changed until the <tt>OK</tt> button is pressed.
 * Exception: the logging preferences take effect immediately, without the need
 * for hitting <tt>OK</tt>.
 */
@ActionID(
        category = "File",
        id = "hirondelle.stocks.quotes.PrefsAction"
)
@ActionRegistration(
        displayName = "#CTL_PrefsAction"
)
@ActionReference(separatorAfter = 75, path = "Menu/File", position = 70)
@NbBundle.Messages("CTL_PrefsAction=Preferences...")
public final class EditUserPreferencesAction extends AbstractAction {
     ...
     * @param aPrefEditors contains implementations of
     * {@link PreferencesEditor}, each of which is placed in a pane of a
     * <tt>JTabbedPane</tt>.
     */
    public EditUserPreferencesAction() {
...
//    Args.checkForNull(aPrefEditors);
        ...
        fPrefEditors = CentralLookup.getDefault().lookup(List.class);
    }
    ...
    /**
     * Specifying this as a field allows for "lazy" creation and use of the GUI,
     * which is of particular importance for a preferences dialog, since they
     * are usually heavyweight, and have a large number of components.
     */
