...
/**
 * Present a dialog to allow user to save all
 * {@link hirondelle.stocks.portfolio.Portfolio} data as a single text file.
 *
 * <P>
 * The text file is not intended to be edited by the end user, but rather as the
 * input to the corresponding {@link ExportAction}. The format of the file is
 * defined by {@link java.util.prefs.Preferences#exportSubtree}. The file name
 * and extension are arbitrary, and defined entirely by the user.
 */
@ActionID(
        category = "File",
        id = "hirondelle.stocks.quotes.ExportAction"
)
@ActionRegistration(
        displayName = "#CTL_ExportAction"
)
@ActionReference(path = "Menu/File", position = 90)
@NbBundle.Messages("CTL_ExportAction=Export...")
...
//    Args.checkForNull(aFrame);
        ...
        putValue(
                LONG_DESCRIPTION,
                "Save all stored Portfolios as an XML file, suitable as input to the Export command."
        );
     ...
     * <li>If an existing file is selected but it has no write access, then the
     * user is informed and asked to select a different file.
     * <li>If an existing file is selected and it has write access, then the
     * user is informed that the file will be overwritten, and is asked to
     * confirm this operation.
     * </ul>
     *
     * <P>
     * User navigation is minimized by storing the apparent directory of current
     * interest. If the user selects this action a <em>second</em> time, then
     * the file selection dialog is pre-set to this directory of interest, so
     * that repeated navigation of the file system is avoided.
     */
    @Override
     ...
     * directory corresponding to the file selection. It is then later used upon
     * any second invocation, to free the user from probably repeating the same
