...
/**
 * Close the {@link CurrentPortfolio}, and display to the user an empty untitled
 * portfolio, which does not need a save.
 * <P>
 * If the <tt>CurrentPortfolio</tt> needs to be saved, then the user is offered
 * the option of saving its edits.
 */
@ActionID(
        category = "File",
        id = "hirondelle.stocks.quotes.CloseAction"
)
@ActionRegistration(
        displayName = "#CTL_CloseAction"
)
@ActionReference(separatorAfter = 35, path = "Menu/File", position = 30)
@NbBundle.Messages("CTL_CloseAction=Close")
public final class FileCloseAction extends AbstractAction implements Observer {

    /**
     * Constructor.
     *
     * @param aCurrentPortfolio will be updated by this action to be untitled
     * and empty of stocks.
     * @param aEditSaver if <tt>aCurrentPortfolio</tt> has any unsaved edits,
     * then
...
//        fEditSaver.save(fCurrentPortfolio, event);
     ...
     * {@link CurrentPortfolio} passed to the constructor. This action is
