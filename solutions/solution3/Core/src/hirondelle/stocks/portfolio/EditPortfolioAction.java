...
@ActionID(
        category = "Edit",
        id = "hirondelle.stocks.quotes.EditPortfolioAction"
)
@ActionRegistration(
        displayName = "#CTL_PortfolioAction"
)
@ActionReference(path = "Menu/Edit", position = 10)
@NbBundle.Messages("CTL_PortfolioAction=Portfolio")
public final class EditPortfolioAction extends AbstractAction {

    /**
     * Constructor.
     *
     * @param aCurrentPortfolio contains the stocks in which the user has an
     * interest.
     * @param aFrame parent frame to which this editor is attached (the main
     * window).
     */
    JFrame aFrame = (JFrame)WindowManager.getDefault().getMainWindow();

    public EditPortfolioAction() {
        super("Portfolio...", UiUtil.getEmptyIcon());
        CurrentPortfolio aCurrentPortfolio = CentralLookup.getDefault().lookup(CurrentPortfolio.class);
     ...
     *
     * <P>
     * If the user selects <tt>OK</tt>, save any edits and update the display of
     * the main window - otherwise, discard all edits.
     *
     * <P>
     * The current stocks are presented in a table, along with
     * <tt>Add-Change-Delete</tt> buttons. The <tt>Add</tt> button is always
     * enabled, while the <tt>Change</tt> and
     * <tt>Delete</tt> buttons are enabled only if a table row is selected.
     *
     * <P>
     * <tt>Add</tt> and <tt>Change</tt> operations will verify input of each
     * field : when focus leaves a field, any invalid input will cause a short
     * message to be placed in the field. Selecting <tt>OK</tt> while any item
     * is still invalid will cause a message to be displayed, asking for
     * correction before proceeding. The only difference between <tt>Add</tt>
     * and <tt>Change</tt> is that the <tt>Change</tt> operation will
     * pre-populate data entry areas.
     */
    @Override
    ...
    // PRIVATE
    /**
     * The current portfolio, whose contents are to be edited. It is important
     * to note that this field is updated only if the user selects OK and if
     * fWorkingCopy does not equal fCurrentPortfolio. All individual edits made
     * by the user are performed on fWorkingCopy. This allows the user to easily
     * back out of any edits they have performed.
     */
     ...
     *
     * <P>
     * Initially contains all items in fCurrentPortfolio; all edits performed by
     * the user will be initially applied only to fWorkingCopy. If the user
     * ultimately chooses OK, then the objects in fCurrentPortfolio are made to
     * match the items in fWorkingCopy.
     *
     * <P>
     * Created once upon construction, and emptied and reused each time
    ...
    /**
     * Create a working copy of the Stock objects in fCurrentPortfolio. It is
     * important to note that since Stock objects are immutable, then creating a
     * copy with independent storage is unnecessary. The <tt>Collection</tt> of
     * items is edited by adding and removing objects themselves, and not by
    ...
    /**
     * Both the change and delete actions should be enabled only if there is a
     * selection made in the table. This private class encapsulates that
     * behavior.
     */
