...
/**
 * Create a new {@link hirondelle.stocks.portfolio.Portfolio} which has a unique
 * name, is placed in storage, and is displayed to the user as the (empty)
 * {@link CurrentPortfolio}, and which does not need a save.
 * 
* <P>
 * If the <tt>CurrentPortfolio</tt> needs to be saved, then the appropriate Save
 * or Save As action is called.
 * 
* <P>
...
@ActionID(
        category = "File",
        id = "hirondelle.stocks.quotes.NewAction"
)
@ActionRegistration(
        displayName = "#CTL_NewAction",
        iconBase = "toolbarButtonGraphics/general/New24.gif"
)
@ActionReferences({
...
public final class FileNewAction extends AbstractAction {
     ...
     */
    public FileNewAction() //  public FileNewAction(
...
//    fEditSaver = aEditSaver;
     ...
     *
     * <P>
     * The supplied name must have a non-zero trimmed length, and must not be
     * the the same name as any currently stored <tt>Portfolio</tt>. If the name
     * entered by the user is invalid, then when <tt>OK</tt> is selected an info
     * box informs the users of the problem, and they are asked to enter another
     * name.
     *
     * <P>
     * If the <tt>CurrentPortfolio</tt> being replaced by this action has any
     * unsaved edits, then allow the user the option of saving such edits.
     */
    @Override
...
//  private EditSaver fEditSaver;
     ...
     * the user hits OK without performing any input, then an empty String is
     * returned.
     */
